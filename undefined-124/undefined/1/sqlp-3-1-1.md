...
2. **클러스터형 인덱스가 없는 상황에서 WITH절에 INDEX (0)을 지정하면 테이블을 Full Scan 하면서 조건절을 필터링한다.INDEX(1)을 지정하면 구문 오류가 발생한다.**
3. **클러스터형 인덱스가 있는 상황에서 WITH절에 INDEX(0) 을 지정하면 클러스터형 인덱스를 Scan 하면서 고객번호를 필터링한다.**
4. **OPTION 절을 이용하려면 힌트를 아래와 같이 기술해야 한다.**
...
1. **클러스터형 인덱스가 없는 상황에서 WITH절에 INDEX (0)을 지정하면 테이블을 Full Scan 하면서 조건절을 필터링한다.INDEX(1)을 지정하면 구문 오류가 발생한다.**
...
1. **오라클은 INDEX\_FS 힌트를 제공하지 않는다. INDEX 힌트로 지정한 인덱스 선두 컬럼이 조건절에 없으면 \_INDEX FULL SCAN이 자동 선택\_되므로 별도 힌트가 필요하지 않다.**
...
1. **두 인덱스 모두 선두 컬럼이 조건절에 없으므로 Index Range Scan은 불 가능하다.**
